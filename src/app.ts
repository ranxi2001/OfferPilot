...
function buildProviders() {
  ...
  if (process.env.OPENAI_API_KEY) {
    ...
    configs.push({
      ...
      models: unique([RECOMMENDED_OPENAI_MODEL, defaultModel, 'gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo']),
      defaultModel,
    });
  }
  ...
  if (process.env.DEEPSEEK_API_KEY) {
    ...
    configs.push({
      provider: new DeepSeekProvider(undefined, process.env.DEEPSEEK_BASE_URL),
      models: unique([defaultModel, 'deepseek-chat', 'deepseek-coder']),
      defaultModel,
    });
  }
